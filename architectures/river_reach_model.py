...
class ReachFlow(nn.Module):
    ...
    def forward(self, river_obj, date_range, tstep, device,
        ...
        if tstep==0:
            ...
            river_obj = self.batched_lateral_inflow_and_storage(river_obj, date_range, tstep, train=False)
            ''' Fix the disappearing gradient connections from these two functions! '''
            # contribution to flow from land at drainage cell
            river_obj = self.batched_terrestrial_flow_contribution(river_obj, tstep, train=False)
            # F_l_out = self.batched_terrestrial_flow_contribution(river_obj, tstep, train=False)
            # for j, rid in enumerate(river_obj.sorted_ids):
                # river_obj.Fl_t[rid] = torch.cat([river_obj.Fl_t[rid], F_l_out[rid]], dim=1)
...
if False:       
    ...
    if tstep==0:
        ...
        if RUN:
            ...
            while n < len(river_obj.sorted_ids):
                ...
                if date_range[tstep].hour==0 and date_range[tstep].minute==0:
                    ...
                    if (n+1)%model.max_batch_size==0 or n==(len(river_obj.sorted_ids)-1):
                        with torch.no_grad() if not train else nullcontext():
                            S_assim = model.s_model(S_inputs) # B, L, C                    
                        
                if (n+1)%model.max_batch_size==0 or n==(len(river_obj.sorted_ids)-1):
                    with torch.no_grad() if not train else nullcontext():
                        LS_latent = model.ls_model(LS_src, LS_trg, mask=None) # B, L, C
                        L_out = model.l_from_ls_model(LS_latent) # B, L, C
                        S_out = model.s_from_ls_model(LS_latent) # B, L, C                
                    ...
                    # store
                    for j, rid in enumerate(batch_rids):            
                        river_obj.S_t[rid] = torch.cat([river_obj.S_t[rid], S_out[j:(j+1),:,:]], dim=1)
                        river_obj.L_t[rid] = torch.cat([river_obj.L_t[rid], L_out[j:(j+1),:,:]], dim=1)
            ...
            river_obj = model.batched_lateral_inflow_and_storage(river_obj, date_range, tstep, train=False)
        ...
        if RUN:
            ...
            while n < len(river_obj.sorted_ids):
                ...
                Fl_s = torch.cat([
                    river_obj.Fl_t[rid][:, :tstep, :],
                    torch.cat([model.zeros_vec, model.ones_vec.expand(-1, tstep-1, -1)], dim=1), # presence indicator   
                    river_obj.L_t[rid][:, :tstep, :],
                    model.indicator_L.expand(-1, tstep, -1),
                    river_obj.r_feats[rid].expand(-1, tstep, -1),
                    model.time_ins[:, :tstep, :]
                ], dim=-1)
                Fl_t = torch.cat([
                ...
                if (n+1)%model.max_batch_size==0 or n==(len(river_obj.sorted_ids)-1):
                    with torch.no_grad() if not train else nullcontext():
                        Fl_latent = model.fstream_model(Fl_src, Fl_trg, mask=None)
                        F_l = model.latent2flow_model(Fl_latent)             
                    
                    # store
                    for j, rid in enumerate(batch_rids):
                        river_obj.Fl_t[rid] = torch.cat([river_obj.Fl_t[rid], F_l[j:(j+1),:,:]], dim=1)
            ...
            river_obj = model.batched_terrestrial_flow_contribution(river_obj, tstep, train=False)
            # F_l_out = model.batched_terrestrial_flow_contribution(river_obj, tstep, train=False)            
            # for j, rid in enumerate(river_obj.sorted_ids):
                # river_obj.Fl_t[rid] = torch.cat([river_obj.Fl_t[rid], F_l_out[rid]], dim=1)
    ...
    if (tstep % nt_opt == 0 and tstep > 0) or tstep == (len(date_range)-1):
        if train:
            ...
            pass
    tstep += 1
    #return river_obj, loss_tstep.item(), model.loss_opt.item(), model.opt_counter
